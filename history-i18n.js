(function(){
  ...
  const D={
    vi:{
      ...
      fieldEstimated:'Giờ dự kiến',fieldActual:'Giờ thực tế',fieldStatus:'Trạng thái',fieldGate:'Cửa',fieldBelt:'Băng chuyền',fieldParking:'Vị trí đỗ',fieldScheduled:'Giờ lịch',fieldBoardingStart:'Bắt đầu boarding',fieldBoardingFinish:'Kết thúc boarding',
      arrivalsKicker:'CHUYẾN ĐẾN',departuresKicker:'CHUYẾN ĐI',onTimeShort:"ĐÚNG GIỜ ≤15'"
    },
    en:{
      ...
      fieldEstimated:'Estimated time',fieldActual:'Actual time',fieldStatus:'Status',fieldGate:'Gate',fieldBelt:'Baggage belt',fieldParking:'Parking bay',fieldScheduled:'Scheduled time',fieldBoardingStart:'Boarding start',fieldBoardingFinish:'Boarding end',
      arrivalsKicker:'ARRIVALS',departuresKicker:'DEPARTURES',onTimeShort:"ON TIME ≤15'"
    },
    ko:{
      ...
      fieldEstimated:'예상 시간',fieldActual:'실제 시간',fieldStatus:'상태',fieldGate:'게이트',fieldBelt:'수하물 벨트',fieldParking:'주기장',fieldScheduled:'예정 시간',fieldBoardingStart:'탑승 시작',fieldBoardingFinish:'탑승 종료',
      arrivalsKicker:'도착',departuresKicker:'출발',onTimeShort:'정시 ≤15분'
    },
    ru:{
      ...
      fieldEstimated:'Расчётное время',fieldActual:'Фактическое время',fieldStatus:'Статус',fieldGate:'Выход',fieldBelt:'Багажная лента',fieldParking:'Место стоянки',fieldScheduled:'Время по расписанию',fieldBoardingStart:'Начало посадки',fieldBoardingFinish:'Окончание посадки',
      arrivalsKicker:'ПРИБЫТИЯ',departuresKicker:'ВЫЛЕТЫ',onTimeShort:'ВОВРЕМЯ ≤15 МИН'
    },
    zh:{
      ...
      fieldEstimated:'预计时间',fieldActual:'实际时间',fieldStatus:'状态',fieldGate:'登机口',fieldBelt:'行李转盘',fieldParking:'停机位',fieldScheduled:'计划时间',fieldBoardingStart:'开始登机',fieldBoardingFinish:'结束登机',
      arrivalsKicker:'到达',departuresKicker:'出发',onTimeShort:'准点 ≤15分钟'
    }
  };

  function t(k,v){let s=(D[lang]&&D[lang][k])||D.vi[k]||k;if(v)Object.keys(v).forEach(x=>s=s.replace(new RegExp('\\{'+x+'\\}','g'),String(v[x])));return s}
  window.JT_HISTORY_T=t;window.JT_HISTORY_LANG=()=>lang;

  const q=s=>document.querySelector(s),qa=s=>[...document.querySelectorAll(s)];
  const set=(sel,val)=>{const el=typeof sel==='string'?q(sel):sel;if(el)el.textContent=val};
  ...
  function applyStatic(){
    ...
    const tables=qa('.history-airline-block table thead tr');tables.forEach(tr=>{const th=tr.querySelectorAll('th');[t('airline'),t('completed'),t('otp'),t('late15'),'OTP15',t('latePct'),t('avg')].forEach((x,i)=>{if(th[i])th[i].textContent=x})});
  ...
  apply();
  if(typeof render==='function')setTimeout(()=>{render();applyStatic()},0);
})();